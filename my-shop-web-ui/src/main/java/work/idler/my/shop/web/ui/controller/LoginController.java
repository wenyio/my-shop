...
import work.idler.my.shop.commons.utils.EmailSendUtils;
import work.idler.my.shop.web.ui.api.UsersApi;
...
public class LoginController {
    ...
    public String login(TbUser tbUser, Model model, HttpServletRequest request) throws Exception{
        ...
        // 登录失败
        if (user == null) {
            model.addAttribute("baseResult", BaseResult.fail("用户名密码错误, 请重新输入!"));
            return "login";
        }

        // 登录成功
        else {
            emailSendUtils.send("用户登录", String.format("用户 [%s] 登录 MyShop", user.getUsername()), "1329208516@qq.com");
            // 将用户信息放入 Session
